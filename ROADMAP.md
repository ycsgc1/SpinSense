...
- The engine **hardcodes the MQTT topics** (`MQTT.Topics.*` and `MQTT.Discovery.Discovery_Topic`): `config.json` exposes the fields but `core/core_engine.py` ignores them. Either wire them up or remove the dead config fields.
