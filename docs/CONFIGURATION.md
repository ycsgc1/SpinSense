...
| **Start the hold when** | A record is played as a unit, so **the album finishes** means nothing is sent while a side is still spinning, then the whole side releases together — which is also when you are most likely to notice a wrong track, since the record is still on. **Each track finishes** starts the clock per song instead. | The album finishes |
| **…then wait (minutes)** | How long after that before plays are submitted. Last.fm has no way to edit or delete a scrobble once sent, so this window is the only chance to catch a wrong identification — delete or correct the play in History during it and it never goes. **Send now** releases everything immediately. | 30 min |
