...
### Added
- **Last.fm scrobbling.** Finished plays are submitted to your profile, plus a live "now playing" indicator. Eligibility is Last.fm's own rule — longer than 30 s, played to half its length or 4 minutes, whichever comes first. Connecting is one click: you go to Last.fm, sign in there, approve SpinSense, and land back on the Settings page connected — your password never touches SpinSense. It works on a LAN with nothing registered up front, because Last.fm's web flow accepts a per-request callback and SpinSense builds one from the address your browser is already using. A manual fallback covers setups where that redirect can't return. Uses your own API key from [last.fm/api/account/create](https://www.last.fm/api/account/create), so the rate limit is yours and nothing inbound is exposed.
