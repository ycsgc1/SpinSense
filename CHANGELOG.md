...
### Added
- **A review window before scrobbling.** Last.fm has no API to edit or delete a scrobble once it's sent, so the only place a wrong identification can be caught is before it goes. Finished plays are now held for 10 minutes by default — delete or correct one in History during that window and it never reaches Last.fm. **Send now** releases everything immediately, held plays included, and the Settings card says how many are waiting and why. Set the hold to 0 for the old behaviour.
